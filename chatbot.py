...
for question in clean_questions:
    ints=[]
    for word1 in question.split():
        if word1 not in questionwords2int:
            ints.append(questionwords2int['<OUT>'])
        else:
            ints.append(questionwords2int[word1])
...
for answer in clean_answers :
    ints=[]
    for word1 in answer.split():
        if word1 not in answerwords2int:
            ints.append(answerwords2int['<OUT>'])
        else:
            ints.append(answerwords2int[word1])
...
def decode_test_set(encoder_state , decoder_cell , decoder_embeddings_matrix , sos_id ,eos_id , maximum_length, num_words ,  sequence_length , decoding_scope , output_function , keep_prob , batch_size ):
      ...
      test_decoder_function = tf.contrib.seq2seq.attention_decoder_fn_inference(output_function,
                                                                                ...
                                                                                attention_values,
                                                                                attention_score_function, 
                                                                                attention_construct_function ,  decoder_embeddings_matrix ,
                                                                                sos_id ,eos_id ,
                                                                                maximum_length, 
                                                                                num_words ,
                                                                                name='attn_dec_inf')        
...
def decoder_rnn(decoder_embedded_input , decoder_embeddings_matrix , encoder_state , sequence_length , rnn_size , num_layers , word2int , keep_prob , batch_size ):
...
def seq2seq_model(inputs, targets , keep_prob , batch_size , sequence_length , answers_num_words , questions_num_words , encoder_embedding_size , decoder_embedding_size , rnn_size , num_layers , questionwords2int):
                  encoder_embedded_input = tf.compat.v1.contrib.layers.embed_sequence(inputs , 
                                                                            answers_num_words + 1 ,
